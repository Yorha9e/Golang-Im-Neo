...
async function runWebRtcSignalingTest() {
  ...
  // 5. 建立双端 WebSocket 连接 (使用全局原生 WebSocket)
  ...
  wsB.onmessage = (event) => {
    try {
      ...
      if (msg.type === 20 /* SIGNALING_OFFER */) {
        bReceivedOffer = true;
        console.log('🎉🎉🎉 [实测成功] User B 成功接收到了后端转发的 SIGNALING_OFFER 信令！');

        // B 立即回复 SIGNALING_ANSWER 给 A
        console.log('➡️ [User B] 正在向 User A 回复 SIGNALING_ANSWER (Type=21)...');
  ...
  wsA.onmessage = (event) => {
    try {
      ...
      if (msg.type === 21 /* SIGNALING_ANSWER */) {
        ...
        console.log('🎉🎉🎉 [实测成功] User A 成功接收到了后端转发的 SIGNALING_ANSWER 信令！');
      }
    } catch (e) {
  ...
  console.log('\n=== 📊 实测结果汇总 ===');
  console.log('B 是否收到 Offer 信令:', bReceivedOffer ? '✅ 是 (后端转发成功)' : '❌ 否 (后端未转发)');
  console.log('A 是否收到 Answer 信令:', aReceivedAnswer ? '✅ 是 (后端转发成功)' : '❌ 否 (后端未转发)');
