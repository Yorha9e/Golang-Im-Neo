...
export const ProfileView: React.FC = () => {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (uploadRes.code === 0 && uploadRes.data.url) {
        await profileApi.updateProfile({ avatar_url: uploadRes.data.url });
        await loadProfile();
      }
    } catch (err) {
  ...
  return (
    <div className="flex-1 h-full overflow-y-auto bg-[#FAF8F5] p-6 lg:p-10">
      <div className="max-w-4xl mx-auto space-y-8">
        {/* 1. 个人资料顶部看板 */}
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          className="relative p-8 rounded-3xl glass-card shadow-warm-md overflow-hidden"
        >
          {/* 背景暖阳微光 */}
            ...
            <div className="flex-1 text-center sm:text-left min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs font-mono text-[#A3927C] mt-0.5">
                    @{username} · UID: {userId || 'u_neo'}
                  </p>
                </div>
              ...
              {isEditingProfile ? (
                ...
                <p className="mt-3 text-sm text-[#8B7355] bg-[#F0EBE3]/50 p-2.5 rounded-xl border border-[#C9B99A]/20 inline-block">
                  {profile?.signature || '「在这个快节奏的世界里，愿有一处角落静谧如暖阳。」'}
                </p>
              )}
            </div>
          </div>
        ...
        >
          ...
          <form onSubmit={handleCreatePost} className="space-y-3">
            <textarea
              ...
              rows={3}
              className="w-full p-3 rounded-2xl glass-input text-xs text-[#2E2419] placeholder-[#A3927C] outline-none resize-none"
            />
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-xs text-[#2E2419]">{post.username}</h4>
                      <p className="text-[10px] text-[#A3927C]">{new Date(post.created_at).toLocaleString()}</p>
                    </div>
                  </div>
                ...
                {/* 动态文本 */}
                {post.content && (
                  <p className="text-xs leading-relaxed text-[#4A3B2C] whitespace-pre-wrap">
                    {post.content}
                  </p>
                )}
                ...
                {/* 图文模式配图 */}
                {post.media_url && (
                  <div className="rounded-2xl overflow-hidden max-h-72 border border-[#C9B99A]/30">
                    <img src={post.media_url} alt="动态配图" className="w-full h-full object-cover" />
                  </div>
                )}
