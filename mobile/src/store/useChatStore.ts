  ...
  loadHistory: async (covId: string, isLoadMore = false) => {
    try {
      ...
      const formattedHistory: ChatMessage[] = rawMessages.map((m: any) => {
        let fromName = m.from_username || '';
        let fromRole = m.from_role || 'user';
        let fromAvatar = m.from_avatar || '';
        if (m.extra) {
          try {
            const parsed = JSON.parse(m.extra);
            if (!fromName && parsed.from_username) fromName = parsed.from_username;
            if (fromRole === 'user' && parsed.from_role) fromRole = parsed.from_role;
            if (!fromAvatar && parsed.from_avatar) fromAvatar = parsed.from_avatar;
          } catch {}
        }
        if (m.from_uid) {
